...
# ---------- Subprocess I/O ----------
def run_cmd(cmd, mode, tmp_path=None, downloads=None):
    global proc
    try:
    ...
    for line in proc.stdout:
        ...
        if mode=="recv" and "received file written to" in low:
            # extract after colon
            real_name = ln.split("to",1)[1].strip().strip("'\"")
...
def receive():
    ...
    tmp_fn = f"wormhole-recv-{int(time.time())}.tmp"
    tmp_path = os.path.join(downloads, tmp_fn)

    status_var.set("Connecting to receive…")
    log(f"Writing to temporary file: {tmp_fn}")
    set_buttons(True); hide_code(); progress_var.set("")

    cmd = ["wormhole","receive","--accept-file","--output-file",tmp_path,code]
    threading.Thread(target=run_cmd, args=(cmd,"recv",tmp_path,downloads), daemon=True).start()
